...
float paddle_y = 800.0f;
float paddle_x = 700.0f;
float paddle_vx = 400.0f;
...
float ball_vx = 400.0f;
float ball_vy = 400.0f;

int box_count = box_rows * box_cols;
...
void handleWinGame(sf::RenderWindow& window, sf::Text& message) {
    displayMessage(window, message, "YOU WON MY FRIEND :)", "green");
    ball_vx = ball_vy = paddle_vx = 0;
}

void handleLoseGame(sf::RenderWindow& window, sf::Text& message) {
    displayMessage(window, message, "YOU LOST MY FRIEND :(", "red");
    ball_vx = ball_vy = paddle_vx = 0;
}
...
int main() {
    sf::RenderWindow window(sf::VideoMode({1400, 900}), "Breakout Game!", sf::Style::Titlebar | sf::Style::Close);
